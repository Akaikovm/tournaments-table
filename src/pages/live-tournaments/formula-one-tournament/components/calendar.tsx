import React from "react";

const Calendar = () => {
  return (
    <div className="text-white">
      <ul className="list-disc list-inside pl-3 mt-4">
        <li>
          <span className="relative right-2">GP De Baréin 5 Marzo</span>
        </li>
        <li>
          <span className="relative right-2">
            GP De Arabia Saudita 19 Marzo
          </span>
        </li>
        <li>
          <span className="relative right-2">GP De Australia 2 Abril</span>
        </li>
        <li>
          <span className="relative right-2">GP De Azerbaiyan 30 Abril</span>
        </li>
        <li>
          <span className="relative right-2">GP De Miami 7 Mayo</span>
        </li>
        <li>
          <span className="relative right-2">GP De Emilia-Romagna 21 Mayo</span>
        </li>
        <li>
          <span className="relative right-2">GP De Monaco 28 Mayo</span>
        </li>
        <li>
          <span className="relative right-2">GP De España 4 Junio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Canada 18 Junio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Austria 2 Julio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Gran Bretaña 9 Julio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Hungria 23 Julio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Belgica 30 Julio</span>
        </li>
        <li>
          <span className="relative right-2">GP De Holanda 27 Agosto</span>
        </li>
        <li>
          <span className="relative right-2">GP De Italia 3 Septiembre</span>
        </li>
        <li>
          <span className="relative right-2">GP De Singapur 17 Septiembre</span>
        </li>
        <li>
          <span className="relative right-2">GP De Japon 24 Septiembre</span>
        </li>
        <li>
          <span className="relative right-2">
            GP De Las Americas 22 Octubre
          </span>
        </li>
        <li>
          <span className="relative right-2">GP De Mexico 29 de Octubre</span>
        </li>
        <li>
          <span className="relative right-2">GP De Brazil 5 de Noviembre</span>
        </li>
        <li>
          <span className="relative right-2">
            GP De Abu Dabi 26 de Noviembre
          </span>
        </li>
      </ul>
    </div>
  );
};

export default Calendar;
